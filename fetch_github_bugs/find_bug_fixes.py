...
def remove_microseconds(date_string):
    ...
    if '+' in date_string:
        date_string = date_string.split('+')[0]  # Keep only the datetime part

    try:
        date_obj = datetime.datetime.strptime(date_string, "%Y-%m-%d %H:%M:%S")
    except ValueError as ve:
        raise ValueError(f"Time data '{date_string}' does not match expected format: {ve}")

    # Format the date as needed, appending UTC offset if desired
